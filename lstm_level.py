...
# the Drop out layer
@jit
def dropout(seed, rate, param):
	isKeep = random.bernoulli(random.PRNGKey(seed), rate, param.shape)
	return jnp.where(isKeep, param / rate, 0)
...
def lstm_seq_dense(params, initCell, initHidden, curInput):
	...
	assert(len(curInput) == params[0].shape[0])
	hiddenInput = jnp.dot(jnp.asarray(curInput).T, params[0]).T
...
def train_step(step_i, opt_state, prevCell, prevHidden, instance, numInputs):
	...
	for tokenI in range(len(instance) - numInputs - 1):
		...
		prevCell, prevHidden = lstm_seq_dense(params, prevCell, prevHidden,
				instance[tokenI : tokenI + numInputs])
...
if (__name__ == "__main__"):
	...
	if (trainAgain):
		W_i = random_params_by_size(numInputs, lstmSize) # will be: inputs \dot W_i
		dense_params = []
		lstm_params = []
