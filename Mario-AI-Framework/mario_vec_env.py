...
class MarioVecEnv(VecEnv):
    """Custom Environment that follows gym interface"""
    metadata = {"render.modes": ["human"]}
    
    def __init__(self):
        super(MarioVecEnv, self).__init__()
        ...
        self.observation_space = spaces.Box(low=0, high=255,
                                            shape=(4, 16, 16), dtype=np.uint8) # 4 frames stacked
        ...
        self.obs = np.zeros([4, 16, 16])


    # TODO: DEFINE - env_is_wrapped, env_method, get_attr, seed, set_attr, step_async, step_wait
