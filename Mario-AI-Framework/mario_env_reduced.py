...
class MarioEnv(gym.Env):
    ...
    def __init__(self, render=True, level = "./levels/original/lvl-1.txt", horizons=False, starts=False, timer = 20, sticky=False, random=False, paths=False, skip=4, max_timestep=165, mario_visibility=7):
        ...
        self.mario_visibility = mario_visibility
    ...
    def step(self, action):
        ...
        if self.random:
            if np.random.random() < 0.02: action = self.action_space.sample()
        ...
        if self.sticky:
            real_action = act if np.random.random() > 0.15 else self.last_action

            # real_action = act if np.random.random() > 0.20 else self.action_space.sample()[0]
        ...
        vx = (info[4] - self.last_x) # reward component for +ve x displacement between steps
        ...
        return self.obs, reward, done, {"vx": a[0], "d":a[1], "f":a[2], "c":a[3], "x":a[4], "g": a[5], "completion": a[6] }
    ...
    def _get_single_obs(self, result):
        ...
        reduced_obs = np.zeros([self.mario_visibility, self.mario_visibility], dtype = np.uint8)
        for i in range(self.mario_visibility):
            for j in range(self.mario_visibility):
                reduced_obs[i][j] = obs[8-(int)((self.mario_visibility-1)/2)+i][8-int((self.mario_visibility-1)/2)+j]
        obs = reduced_obs
        ...
        # obs[obs == 60] = 25 # mushroom top
        # obs[obs == 61] = 25 # mushroom norm
        # obs[obs == 62] = 25 # mushroom top
        # obs[obs == 63] = 25 # mushroom norm
        ...
        scale = np.floor(84/self.mario_visibility)
        remainder = (int)(84-self.mario_visibility*scale)
        resize = (int)(scale*self.mario_visibility)


        obs = cv2.resize(obs, [resize, resize], interpolation=cv2.INTER_AREA)
        obs = np.pad(obs, [ [(int)(remainder/2), remainder-(int)(remainder/2)], [(int)(remainder/2), remainder-(int)(remainder/2)] ] )
